"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("angular2/core");
var http_1 = require("angular2/http");
var local_storage_1 = require("../service/local-storage");
var Observable_1 = require("rxjs/Observable");
var HttpClient = (function () {
    function HttpClient(_http, _storageService) {
        this._http = _http;
        this._storageService = _storageService;
    }
    HttpClient.prototype.setHeader = function (requestArgs) {
        var finalHeader;
        var requestOption;
        if (requestArgs) {
            finalHeader = new http_1.Headers(requestArgs.headers);
            requestOption = requestArgs;
        }
        else {
            finalHeader = new http_1.Headers();
            requestOption = {};
        }
        var token = this._storageService.getFromLocalStorage("token");
        finalHeader.append('Authorization', 'Bearer ' + 'abcd.eq');
        finalHeader.append("Content-Type", "application/json");
        finalHeader.append("Accept", "application/json");
        requestOption.headers = finalHeader;
        return requestOption;
    };
    HttpClient.prototype.get = function (url, options) {
        var requestOption = this.setHeader(options);
        return this._http.get(url, requestOption);
    };
    HttpClient.prototype.post = function (url, body, options) {
        var requestOption = this.setHeader(options);
        console.log(requestOption.headers.get("Authorization"));
        console.log("body", body.toLocaleString());
        return this._http.post(url, body, requestOption).map(function (response) { return response.json(); })
            .do(function (data) {
            console.log("request", requestOption.headers.toJSON());
            console.log("data ", data);
        })
            .catch(this.handleError);
    };
    HttpClient.prototype.handleError = function (error) {
        console.log(error.json());
        throw Observable_1.Observable.throw(error.json() || 'Validation failed.');
    };
    HttpClient = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [http_1.Http, local_storage_1.LocalStorgeService])
    ], HttpClient);
    return HttpClient;
}());
exports.HttpClient = HttpClient;
//# sourceMappingURL=http.client.js.map